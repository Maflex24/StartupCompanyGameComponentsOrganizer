...
components.OrderBy(c => c.Profession).ThenBy(c => c.EmployeeLevel);
components.OrderBy(c => c.Profession).ThenBy(c => c.EmployeeLevel);
...
while (true)
{
    ...
    {
        Console.WriteLine($"{i.ToString().PadRight(3)} {components[i].Name.PadRight(28)} | {components[i].Profession} {components[i].EmployeeLevel}");
    }

    Console.Write("Type the number of component to show path: ");
    var userChoice = int.Parse(Console.ReadLine());

    Console.WriteLine($"Producing flow for [{components[userChoice].Name}]");

    components[userChoice].Produce();
...
//var storageModule = components.SingleOrDefault(c => c.Name == "Storage Module");
//storageModule.Produce();
